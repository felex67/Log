...
/** __zipper::entry_base */
        ...
        ssize_t entry_base::__from_string(const char* Source, const char *mask = nullptr) { return -1; };
        ssize_t entry_base::__to_string(std::string &Dest, const char *mask = nullptr) { return -1; };
        entry_base::operator bool () { return data != 0; }

/** __zipper::instance */
        instance::instance(const char *Name, const char *Path, const size_t ByteCnt)
            : __base(INSTANCE, Name, (ByteCnt - sizeof(*this)) / sizeof(entry_base))
            , path(nullptr)
        {
